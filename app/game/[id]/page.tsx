...
function GamePageContent() {
  ...
  const safeSetGame = (newGame: any) => {
    if (!newGame) {
      console.warn('Attempted to set null/undefined game, ignoring');
      return;
    }
    
    // Validate that the game has a proper state
    if (!newGame.state || typeof newGame.state !== 'object') {
      console.error('Invalid game state detected, ignoring update:', newGame);
      return;
    }
    
    // Ensure critical state properties exist
    const requiredStateProps = ['phase', 'scores', 'isRunning', 'elapsedMs'];
    const missingProps = requiredStateProps.filter(prop => !(prop in newGame.state));
    
    if (missingProps.length > 0) {
      console.error('Missing critical state properties, ignoring update:', missingProps, newGame.state);
      return;
    }
    
    // If we get here, the game state is valid
    setGame(newGame);
  };
  
  // Redirect state management - prevent navigation during render
  const [shouldRedirect, setShouldRedirect] = useState<string | null>(null);
  const [shouldRedirectToView, setShouldRedirectToView] = useState(false);
  
  // Refs for debounced sync
  const syncTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const pendingSyncRef = useRef<any>(null);

  // Error handling state - must be at the top level
  const [errorRetryCount, setErrorRetryCount] = useState(0);
  const [showError, setShowError] = useState(false);

  // Handle redirects in useEffect to avoid calling router.push during render
  useEffect(() => {
    if (shouldRedirect) {
  ...
  }, [shouldRedirectToView, id, router]);
  ...
  // Now define advancePhase AFTER isExpired is available
  ...
  useEffect(() => {
    if (!game || !user || errorRetryCount >= 3) return;
    
    if (!game?.state || !game?.state?.phase || !game?.state?.phase?.type) {
      const timer = setTimeout(() => {
        const reloadGame = async () => {
          try {
            const loadedGame = await loadGame(id);
            if (loadedGame && loadedGame.state && loadedGame.state.phase && loadedGame.state.phase.type) {
              setGame(loadedGame);
              setShowError(false);
              return;
            }
          ...
          // If we get here, the reload failed
  ...
  if (shouldRedirect || shouldRedirectToView) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
  ...
  // Safety check for game state - be more lenient to avoid false positives during updates
  if (!game?.state || !game?.state?.phase || !game?.state?.phase?.type) {
    // Only show error if we've tried to recover and still have issues
    // The error handling state is now at the top level
    if (showError || !user) {
      console.error('Game state is missing or incomplete after recovery attempts:', game);
      return (
        <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 flex items-center justify-center">
          <div className="text-center">
            <div className="text-red-400 text-6xl mb-2">⚠️</div>
  ...
  // Only log game state in development mode
  ...
  async function score(team: "A"|"B") {
    // Safety check: ensure state exists and is valid before proceeding
    if (!game?.state || !game.state.phase || !game.state.scores) {
      console.error('Cannot score: invalid game state');
      return;
    }
    
    console.log('score called for team:', team, 'current scores:', game.state.scores, 'current centre pass:', game.state.centrePass);
    
    // Update quarter scores
    const currentQuarter = game.state.phase.index;
    const currentQuarterScores = game.state.quarterScores?.[currentQuarter] || { A: 0, B: 0 };
    const updatedQuarterScores = {
      ...game.state.quarterScores,
      [currentQuarter]: {
  ...
  async function startPause() {
    ...
    console.log('startPause called, current state:', { isRunning: game.state.isRunning, phase: game.state.phase });
  ...
  async function resetTimer() {
    ...
    if (!game?.state) {
      console.error('Cannot reset timer: game state is not loaded yet');
      return;
    }
    const updatedGame = await updateGameOptimized(id, {
      state: {
          ...
          <div className="grid grid-cols-3 items-center text-center gap-4">
            ...
            <div className="space-y-4">
              ...
              <button 
                ...
                onClick={() => {
                  ...
                  if (game?.state?.isRunning && game?.state?.phase?.type === "quarter") {
                    score("A");
                  }
                }}
                disabled={!game?.state?.isRunning || game?.state?.phase?.type !== "quarter"}
              >
                +1 GOAL
              </button>
            </div>
            ...
            <div className="flex flex-col space-y-2">
              ...
              {!game?.state?.isRunning && (
                <button 
                                  className={`font-bold py-2 px-4 rounded-lg transition-all duration-200 text-sm ${
                  game?.state?.phase?.type === "fulltime"
                    ? 'bg-gray-400 text-gray-300 cursor-not-allowed opacity-50'
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <button 
              className={`font-bold py-3 px-4 rounded-lg transition-all duration-200 text-sm ${
                (game?.state?.phase?.type === 'fulltime' || !game?.state?.isRunning)
              ...
              disabled={game?.state?.phase?.type !== 'fulltime' && game?.state?.isRunning}
            >
              {game?.state?.phase?.type === 'fulltime' ? 'START NEW' : 'RESET GAME'}
            </button>
                    ...
                    <div class="bg-slate-900 rounded-xl p-4 mb-4 border border-slate-600/30">
                      ...
                      <div class="flex justify-center items-center space-x-8">
                        <div class="text-center">
                          ...
                          <div class="text-white text-3xl font-bold">${game?.state?.scores?.A || 0}</div>
                        </div>
